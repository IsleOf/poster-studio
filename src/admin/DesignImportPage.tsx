...
import { createDesignGroup, getDesignGroups, deleteDesignGroup, getListings } from './adminApi';

// All supported size codes + display info
const ALL_SIZES = [
    // Imperial
...
// Default fulfillment provider per size (cheapest from our DB research)
const DEFAULT_FULFILLMENT: Record<string, { provider: string; costCents: number }> = {
    '4x6':    { provider: 'shortrunposters', costCents: 713 },
    '5x7':    { provider: 'shortrunposters', costCents: 713 },
    '8x10':   { provider: 'shortrunposters', costCents: 755 },
    '11x14':  { provider: 'shortrunposters', costCents: 777 },
    '12x16':  { provider: 'shortrunposters', costCents: 900 },
    '16x20':  { provider: 'shortrunposters', costCents: 827 },
    '18x24':  { provider: 'shortrunposters', costCents: 1000 },
    '20x28':  { provider: 'shortrunposters', costCents: 2467 },
    '24x36':  { provider: 'scalablepress',   costCents: 1120 },
    '30x40':  { provider: 'sharpprints',     costCents: 4500 },
    '36x48':  { provider: 'persqft',         costCents: 7000 },
...
// Suggest a retail price (3× COGS, rounded to .99)
...
const BASE_DESIGN_SETTINGS = {
    ...
    maskShape: 'circle',
    posterColor: '#f0efe9',
    mapInteriorColor: '#0c0f16',
    textColor: '#0c0f16',
    starColor: '#e8e8e8',
    isLightMode: true,
    titleFont: 'Details001',
    subtitleFont: 'Title001',
    detailsFont: 'Title001',
    dedicationFont: 'Title001',
    titleFontSize: 72,
    subtitleFontSize: 28,
    detailsFontSize: 22,
    dedicationFontSize: 20,
    titleKerning: 6,
    subtitleKerning: 2,
    detailsKerning: 3,
    dedicationKerning: 2,
    showBorder: true,
    borderStyle: 'fineline',
    shapeOutlineWidth: 2,
...
export default function DesignImportPage() {
    ...
    const [settingsJson, setSettingsJson] = useState(JSON.stringify(BASE_DESIGN_SETTINGS, null, 2));
    ...
    const [sizeRows, setSizeRows] = useState<Record<string, SizeRow>>(() => {
        ...
        for (const s of ALL_SIZES) {
            ...
            rows[s.code] = {
                enabled: false,
                mockup_path: '',
                fulfillment_provider: def?.provider || 'shortrunposters',
    ...
    // Auto-fill mockup paths when mockupBase changes
    useEffect(() => {
        ...
        setSizeRows(prev => {
            const next = { ...prev };
            for (const s of ALL_SIZES) {
                next[s.code] = { ...next[s.code], mockup_path: `${base}/${s.code}.png` };
            }
            return next;
        });
    }, [mockupBase]);
    ...
    const updateRow = (code: string, field: keyof SizeRow, value: any) => setSizeRows(prev => ({ ...prev, [code]: { ...prev[code], [field]: value } }));
    ...
    return (
        <Box p={6} maxW="1100px">
            ...
            <Text color="gray.500" fontSize="sm" mb={6}>
                Create a design group with multiple size variants. Each size becomes a template added to the listing.
            </Text>
            ...
            <Accordion allowMultiple defaultIndex={[0, 1, 2]}>
                ...
                <AccordionItem border="1px solid" borderColor="gray.200" borderRadius="md" mb={3}>
                    ...
                    <AccordionPanel p={4}>
                        <VStack spacing={3} align="stretch">
                            <HStack>
                                <FormControl>
                                    <FormLabel fontSize="xs">Design ID <Text as="span" color="gray.400">(slug format, e.g. sm001-design001)</Text></FormLabel>
                ...
                <AccordionItem border="1px solid" borderColor="gray.200" borderRadius="md" mb={3}>
                    <AccordionButton bg="gray.50" borderRadius="md" _expanded={{ bg: 'blue.50' }}>
                        <Box flex="1" textAlign="left" fontWeight="semibold" fontSize="sm">3 — Design Settings (JSON)</Box>
                        <AccordionIcon />
                    </AccordionButton>
                    <AccordionPanel p={4}>
                        <Text fontSize="xs" color="gray.500" mb={2}>
                            Configure the design in the poster designer first, then copy the settings here. Or edit JSON directly.
                        </Text>
                        <Textarea
                            size="sm"
                            fontFamily="mono"
                        ...
                        />
                        {settingsError && <Alert status="error" mt={2} py={1} fontSize="xs"><AlertIcon />{settingsError}</Alert>}
                    </AccordionPanel>
                </AccordionItem>
                ...
                <AccordionItem border="1px solid" borderColor="gray.200" borderRadius="md" mb={3}>
                    ...
                    <AccordionPanel p={4}>
                        <FormControl mb={3}>
                            <FormLabel fontSize="xs">Mockup base path <Text as="span" color="gray.400">(auto-fills per-size paths, e.g. /designs/SM001/Design001)</Text></FormLabel>
                                            ...
                                            <Tr key={s.code} bg={row.enabled ? 'green.50' : undefined} opacity={row.enabled ? 1 : 0.5}>
                                                ...
                                                <Td>
                                                    <NumberInput
                                                        ...
                                                        onChange={(_, v) => updateRow(s.code, 'sell_price_cents', Math.round(v * 100))}
                                                        isDisabled={!row.enabled}
                                                        min={0}
                                                        step={5}
                                                        w="100px"
